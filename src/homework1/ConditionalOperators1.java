package homework1;

import java.util.Scanner; // importing of the class Scanner from java.util package

public class ConditionalOperators1 {
    public static void main(String[] args) {
        Scanner in = new Scanner(System.in); // create an object of the class Scanner
        System.out.print("Input name: ");
        String name = in.nextLine(); // get the input and assign it to the variable name
        in.close();
        if (name.equals("Вася")) {
            System.out.println("Привет!");
            System.out.println("Я тебя так долго ждала.");
        } else {
            if (name.equals("Анастасия")) {
                System.out.print("Я тебя так долго ждала.");
            } else {
                System.out.print("Добрый день, а Вы кто?");
            }
        }

    }
}
